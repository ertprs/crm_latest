	...
	function addholiday() { 
		...
		$holidays=implode(',',$normal_holidays);
			...
			if (!$hasError) {
				...
				if ($todos) {
					$allactivestaff = $this->Staff_Model->get_all_staff_active();
					if(!empty($allactivestaff )){
						foreach($allactivestaff  as $userInfo){
					$template = $this->Emails_Model->get_template('staff', 'staff_holiday');
			...
			$clean = $this->security->xss_clean( $email );
				...
				'{end_date}' => $enddate,
				'{holiday_description}'=>$_POST[ 'detail' ],

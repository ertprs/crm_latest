...
class Document extends CIUIS_Controller {
	...
	function create() {
		if ( $this->Privileges_Model->check_privilege( 'material', 'create' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				...
				$checkinterval= $this->input->post('checkinterval');
				$checkinterval=isset($checkinterval)?$checkinterval:0;
				if($this->input->post('intervalvalue1')!='')
					$interval=$this->input->post('intervalvalue1');
				else if($this->input->post('intervalvalue2')!='')
					$interval=$this->input->post('intervalvalue2');
				else if($this->input->post('intervalvalue3')!='')
					$interval=$this->input->post('intervalvalue3');
				else if($this->input->post('intervalvalue4')!='')
					$interval=$this->input->post('intervalvalue4');
				if($checkinterval==0){
					$interval='';
				}
				$checkremind= $this->input->post('checkremind');
				$checkremind=isset($checkremind)?$checkremind:0;
				$remind_staff= $this->input->post('remind_staff');
				if($checkremind==0){
					$remind_staff='';
				}
				$contactdetails= $this->input->post('contactdetails');
				$contactdetails=isset($contactdetails)?$contactdetails:0;
				...
				if (!$hasError) {
					...
					$expiry_date=date('Y-m-d H:i:s A',strtotime($this->input->post( 'expiry_date' )));
					...
					$params = array(
						...
						'repeat_interval' => $checkinterval,
						'interval_type' => $interval,
						'remind_staff' =>$checkremind,
						'remind_staff_id' => $remind_staff,
	...
	function update(){
   	...
   	if ( isset( $_POST ) && count( $_POST ) > 0 ) {
		...
		$contactdetails1= $this->input->post('contactdetails');
		$contactdetails=isset($contactdetails1)?$contactdetails1:0;
		$status = $this->input->post( 'status' );

...
function get_client_contacts(){
	...
	$contact= ' <select class="form-control contact my-select" data-live-search="true"  name="client_contact_id" id="client_contact_id"><option value="">Select Contact</option>';
...
$alldata='';
	...
	$data['supplier_details']=$supplier_details=explode(',',$salestea);
		...
		if($proposals) {
			if ( $this->Privileges_Model->check_privilege( 'estimations', 'edit' ) ) {
				...
				$before_main_items_count= $this->db->get_where( 'estimations_main_items', array( 'estimation_id' => $id) )->num_rows();
				$before_sub_items_count= $this->db->get_where( 'estimations_sub_items', array( 'estimation_id' => $id) )->num_rows();
				$before_estimation_total_cost_amt= $proposals['estimation_total_amt'];
				...
				if(sizeof($order) > 0) {
					if($order['status_id'] == '1' || $order['status_id'] == '3' || $order['status_id'] == '7'){
					 $response = $this->db->where( 'id', $order['id'] )->update( 'orders', array( 'content' =>$special_notes));
					}
				...
				'estimation_id'=>$estimation_id);
				 ...
				 $after_main_items_count= $this->db->get_where( 'estimations_main_items', array( 'estimation_id' => $id) )->num_rows();
				 $after_sub_items_count= $this->db->get_where( 'estimations_sub_items', array( 'estimation_id' => $id) )->num_rows();
				 
				 if($after_main_items_count != $before_main_items_count){
					if($after_main_items_count > $before_main_items_count){
						$historyparams=array('module'=>'estimations','relation_id'=>$estimation_id,'type'=>'added Item');
						$this->Privileges_Model->create_history($historyparams);
					}else{
						$historyparams=array('module'=>'estimations','relation_id'=>$estimation_id,'type'=>'deleted Item');
						$this->Privileges_Model->create_history($historyparams);
					}
				 }
				 if($before_sub_items_count != $after_sub_items_count){
					 if($after_sub_items_count > $before_sub_items_count){
						$historyparams=array('module'=>'estimations','relation_id'=>$estimation_id,'type'=>'added Material');
						$this->Privileges_Model->create_history($historyparams);
					}else{
						$historyparams=array('module'=>'estimations','relation_id'=>$estimation_id,'type'=>'deleted Material');
						$this->Privileges_Model->create_history($historyparams);
					}
				 }
				 
				 if($before_estimation_total_cost_amt != $estimation_total_cost_amt){
					$historyparams=array('module'=>'estimations','relation_id'=>$estimation_id,'type'=>'modified value');
					$this->Privileges_Model->create_history($historyparams);
				 }
	...
	function view( $id ) {
		...
		$data['estimateByStaff'] = ($est[ 'user_id' ] !='0' ? $this->Orders_Model->get_staff_name_by_id($est[ 'user_id' ]) : '');
		$data['estimateDate'] =($est[ 'user_id' ] !='0' ? date('d-m-Y g:i a', strtotime( $est['created'])) : '');

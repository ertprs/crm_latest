	...
	function create() { 
		if ( $this->Privileges_Model->check_privilege( 'expenses', 'create' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				...
				$customer_id1 = $this->input->post('customer_id');
				if($customer_id1==''){
					$customer_id=0;
						$params = array(
				...
				} else if (($amountType=='1')&& $account_id == '') {
					$hasError = true;
					$data['message'] = lang('selectinvalidmessage'). ' ' .lang('account');
				}else if (($amountType=='2')&& $card_id == '') {
					$hasError = true;
					$data['message'] = lang('selectinvalidmessage'). ' ' .lang('Card');
				}
				...
				if ( isset( $_POST ) ) {
					...
					if (count($_FILES['file_name']['name'])>0) {
						...
						foreach ($_FILES['file_name']['name'] as $key => $val) {
							...
							$ext = explode(".", $name);
							$image_name = preg_replace("/[^a-z0-9\_\-\.]/i", '', basename($_FILES['file_name']['name'][$key]));
							//$image_name = "expense-".$size.rand(0,5000000).".".end($ext);
							//image_name = $name;
							if(move_uploaded_file($temp,"uploads/files/expenses/".$id."/".$image_name)){
								$params = array(
									'relation_type' => 'expense',
									'relation' => $id,
				...
				if ( isset( $_POST ) ) {
					...
					if ($countfiles>0) {
						for($i=0;$i<$countfiles;$i++) {
							...
							$name = $_FILES['file_name'.$i]['name'];
							$size = $_FILES['file_name'.$i]['size'];
							$type = $_FILES['file_name'.$i]['type'];
							$temp = $_FILES['file_name'.$i]['tmp_name'];
							
							$ext = explode(".", $name);
							$image_name = preg_replace("/[^a-z0-9\_\-\.]/i", '', basename($_FILES['file_name'.$i]['name']));
							//$image_name = "expense-".$size.rand(0,5000000).".".end($ext);
							//image_name = $name;
							if(move_uploaded_file($temp,"uploads/files/expenses/".$id."/".$image_name)){
								$params = array(

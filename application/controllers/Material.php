...
function update(){
 ...
 // echo '<pre>'; print_r($_POST); die;
	...
	function get_all_materials_new() {
		if ( $this->Privileges_Model->check_privilege( 'material', 'all' ) ) {
			$staffs = $this->Material_Model->get_all_materials();
			$data_staffs = array();
			foreach ( $staffs as $staff ) {
				...
				$data_staffs[] = array(
					'id' => $staff[ 'material_id' ],
					'code' => $staff[ 'item_code' ],
					'name' => $staff[ 'itemname' ],
					'description' => $staff[ 'itemdescription' ],
					'remarks' => $staff[ 'remarks' ],
					'unit' => $staff[ 'unit_name' ],
					'cost' => $staff[ 'cost' ],
					'tax'=>5,
...
echo '<div class="form-group col-md-6"><a href="javascript:void(0);" class="add_button1" title="Add field"><i class="fa fa-plus-circle text-success" style="font-size: 24px;"></i></a> <label for="inputZip">Add More Vendor</label></div>';
 echo ' <input type="submit" class="btn btn-success col-md-12"  value="Update">
	...
	//////Code By Namrta//////
	function download($user_id,$userfile) {
        if (isset($userfile)) {
           // $fileData = $this->Document_Model->get_file_new($userfile);
            if (is_file('./Uploads/images/' . $userfile)) {
                $this->load->helper('file');
                $this->load->helper('download');
                $data = file_get_contents('./Uploads/images/'.$userfile);
                force_download($userfile, $data);
            } else {
                $this->session->set_flashdata('ntf4', lang('filenotexist'));
                redirect('material/matview/' . $user_id);
            }
        }
    }	
	function delete_file($id,$image_name){
	    $data = $this->Material_Model->update_file($image_name,$id);
	    echo json_encode($data); 
	    
	}
	function add_file( $material_id){
		if(!empty(array_filter($_FILES['files']['name']))){
			foreach($_FILES['files']['name'] as $key=>$val){
				...
				if(in_array($ext,$allowed)){
					...
					$newfilename =  "custom_file_".rand(100000,200000).time().".".($ext);
					//echo $newfilename = 'custom_file_'.rand().'.'.$ext;
			...
			if($old_imgs !="") $docs = array($old_imgs->documents); 
			 ...
			 if(isset($Return) && (!empty($Return))){
				$ov_docs = array_merge($docs,$Return);
			}else{
				$ov_docs = $docs;
			}
			$mat_imgs = implode(',',$ov_docs);
		...
		redirect(base_url().'material/matview/'.$material_id);
   	}

...
function get_material_data(){
	...
	function get_request_data(){
	    $data['user_id'] = $this->session->usr_id;
		if ( $this->Privileges_Model->check_privilege( 'notebooks', 'all' ) ) {
			$notebooks= $this->Notebooks_Model->get_all_notebooks();
		} else if( $this->Privileges_Model->check_privilege( 'notebooks', 'own')){
			$notebooks= $this->Notebooks_Model->get_notebooks_by_userid($this->session->usr_id);
		} else {
			$this->session->set_flashdata( 'ntf3',lang( 'you_dont_have_permission' ) );
			redirect(base_url('notbooks'));
		}
		$data_staffs = array();
		foreach ( $notebooks as $notebook ) {
			$page_name = $this->Notebooks_Model->get_page_names($notebook[ 'notebook_id' ]);
			$data_staffs[] = array(
				'id' => $notebook[ 'notebook_id' ],
				'staffavatar' => $notebook['staffavatar'],
				'notebook_list' => $notebook[ 'notebook_list' ],
				'created_date' => date('d-M-Y',strtotime($notebook['created_date'])),
				'page_names' => $page_name['page_names'],
			);
		};
		echo json_encode( $data_staffs );
    /*	$searchdata = $this->input->post('data');
		$id = $this->session->usr_id;
		$response =$this->notebooks_lib->getNoteBookList($searchdata,$id);
		return $response;*/
	}

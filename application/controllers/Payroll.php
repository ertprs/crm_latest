...
function payroll_excel($from_date,$to_date){
...
//$payslip_data =	$this->Payroll_Model->get_payslip_data($from_date,$to_date);
	...
	$payslip_data = $this->Payroll_Model->getrecords($from_date,$to_date);
...
$table_columns = array("Employee Name", "Period", "No. of Days", "Absents", "Workshift Hours", "Working Hours", "Over Time", "Salary");
 ...
 $object->getActiveSheet()->mergeCells('C6:C7');
    ...
    $object->getActiveSheet()
    ->getCell('E6')
    ->setValue('Workshift Hours'); 
    
     $object->getActiveSheet()->mergeCells('F6:F7');
    $object->getActiveSheet()
    ->getCell('F6')
    ->setValue('Working Hours');
...
$object->getActiveSheet()->getStyle($exrow11,5)->applyFromArray($styleArray);
      ...
      $emp_data = $this->Payroll_Model->get_salary_info_details($row['staff_id']);
   $object->getActiveSheet()->setCellValueByColumnAndRow(0, $excel_row, $emp_data['staffname']);
   $object->getActiveSheet()->setCellValueByColumnAndRow(1, $excel_row, $from_date.'-'.$to_date);
  // $permit_no = strval($emp_data['work_permit_no']);
   $object->getActiveSheet()->setCellValueByColumnAndRow(2, $excel_row, $row['present_days']);


   $object->getActiveSheet()->setCellValueByColumnAndRow(3, $excel_row, $row['lop_days']);
   
   
   $object->getActiveSheet()->setCellValueByColumnAndRow(4, $excel_row,'240');
  
  $working_hrs =  8*$row['present_days'];
    $object->getActiveSheet()->setCellValueByColumnAndRow(5, $excel_row,$working_hrs);
   
  
    $object->getActiveSheet()->setCellValueByColumnAndRow(6, $excel_row, $row['ot_hours']);

...
class Projects extends CIUIS_Controller {
	...
	function project( $id ) {
		...
		if ($this->Privileges_Model->check_privilege( 'projects', 'all') ) {
			...
			$approvalAccess=$this->Privileges_Model->has_approval_access('projects');
			$maxvalue=$approvalAccess['maxvalue'];
			$comperKey='total';
			if($approvalAccess['type']=='level'){
				$data['projects']=check_approval_data_ForId($project,$maxvalue,'Level');
			}else if($approvalAccess['type']=='price'){
				$data['projects']=check_approval_data_ForId($project,$maxvalue,$comperKey);
			}else{
				$data['projects']=check_approval_data_ForId($project,$maxvalue,'NotAccess');
			}
			...
			if ( isset( $_POST[ 'subproject' ] ) ) {
				...
				if($subPrjData['completedCnt'] == $subPrjData['totalRowsCnt']) {
					...
					$this->db->where( 'id', $projectid );
	 				$this->db->update('projects', array('status_id'=>5));
				} else {
					$this->db->where( 'id', $projectid );
	...
	function uncompletesubprojects() {
		if ( $this->Privileges_Model->check_privilege( 'projects', 'edit' ) ) {
			if ( isset( $_POST[ 'subprojectid' ] ) ) {
				...
				} else if($subPrjData['percentageCompetion'] != 100){
					$this->db->where( 'id', $projectid );
	 				$this->db->update('projects', array('status_id'=>2));
				}
			}

...
class Purchases extends CIUIS_Controller {
	...
	///Code By Namrta///////
	function get_VendorData($vendor){
	
		$vendors = array();
		$vendors = $this->Vendors_Model->get_vendors($vendor);
		$data_vendors = array();
		$credit_period='';
		if($vendors['credit_period']==''){
			$credit_period=30;
		}else{
			$credit_period=(int)$vendors['credit_period'];
		}
		echo json_encode( $credit_period );
	}
}

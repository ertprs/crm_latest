...
class Staff extends CIUIS_Controller {
	...
	function index() {
		...
		if ( !$this->Privileges_Model->has_privilege( $path ) ) {
			...
			if($this->Privileges_Model->check_privilege('staff', 'own')){
				$data[ 'staff' ] = $this->Staff_Model->get_all_staff('',$this->session->userdata( 'usr_id' ));
			}else{
			$data[ 'staff' ] = $this->Staff_Model->get_all_staff();
			}
		
			$data[ 'settings' ] = $this->Settings_Model->get_settings_ciuis();
			$data[ 'departments' ] = $this->Settings_Model->get_departments();if ($this->isAdmin()) {
			    $data['editdepartment']='1';
			}else{
			    $data['editdepartment']='0';
			}
			$path = $this->uri->segment( 1 );
			if ( !$this->Privileges_Model->has_privilege( $path ) ) {
				$this->session->set_flashdata( 'ntf3', '' . lang( 'you_dont_have_permission' ) );
				...
				if (!$hasError) {
					...
					if ($staff_id) {
						...
						if ($template['status'] == 1) {
							...
							$this->mail->send_email($this->input->post( 'email' ), $template['from_name'], $subject, $message);
						}
						$data['success'] = true;
    ...
    function staffmember( $id ) {
		if ( $this->Privileges_Model->check_privilege( 'staff', 'all' ) || $this->Privileges_Model->check_privilege( 'staff', 'own' ) ) {
			...
			if ( isset( $staff[ 'id' ] ) ) {
				$data[ 'id' ] = $staff[ 'id' ];
					$data[ 'staffres' ] = $staff;
					$data[ 'staffdetails' ] = $staffdetails=$this->Staff_Model->get_all_staff_active();
...
*/
	...
	function lock_department( $id ) {
		
			if (isAdmin()) {
				$departments = $this->Settings_Model->get_department( $id );
				if ( isset( $departments[ 'id' ] ) ) {
					$status=$departments[ 'lock_status' ];
					if($status==0){
						$updatestatus=1;
					}else{
						$updatestatus=0;
					}
						$this->Settings_Model->status_change_department($id,$updatestatus);
						$data['message'] = lang('department').' '.lang('lockdepartment');
						$data['success'] = true;
						echo json_encode($data);
				}
			} else {
	...
	function get_staff($type="") {
		if ( $this->Privileges_Model->check_privilege( 'staff', 'all' ) ) {
			...
			$staffs = $this->Staff_Model->get_all_staff_own($type,$this->session->userdata( 'usr_id' ));
		}
	...
	function update_leaves($id)
	{
		...
		if($this->input->server("REQUEST_METHOD")=="POST"){ 
			$data = $this->input->post();
				$ls_date = $this->input->post('leave_start_date');
		      $rj_date = $this->input->post('rejoin_date');
		    $leave_type =  $this->input->post('type_of_leave');

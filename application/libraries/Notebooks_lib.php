...
class Notebooks_lib{
	...
	function getNoteBookList($searchdata='',$id) {
		$notebooksData=array();
		$finaldata='';
		if($searchdata==''){
			if ( $this->CI->Privileges_Model->check_privilege( 'notebooks', 'all' ) ) {
			 $notebooksData= $this->CI->Notebooks_Model->get_all_notebooks();
				...
				$notebooksData = $this->CI->Notebooks_Model->get_notebooks_by_userid($id);
			...
			if ( $this->CI->Privileges_Model->check_privilege( 'notebooks', 'all' ) ) {
			  $notebooksData= $this->CI->Notebooks_Model->get_all_notebooks_bysearch($searchdata,'1');
				...
				$notebooksData = $this->CI->Notebooks_Model->get_all_notebooks_bysearch($searchdata,$id);
			} else {
				$notebooksData = array();
			}
		}

...
class Expenses_Model extends CI_Model {
	...
	function all_expenses( $id, $staff_id='' ) {
		$this->db->select( '*,expenses.created,vendors.company as customer,vendors.type as type,vendors.namesurname as individual,vendors.email as customeremail,vendors.phone as customer_phone, accounts.name as account,expensecat.name as category,staff.staffname as staff,staff.id as staff_id, expenses.description as desc, expenses.id as id, recurring.id as recurring_id, recurring.status as recurring_status, recurring.relation_type as recurring_relation_type, recurring.period as recurring_period, recurring.type as recurring_type, recurring.end_date as recurring_endDate, vendors.country_id, vendors.taxoffice as customer_tax, vendors.taxnumber as customer_taxnum' );
		$this->db->join( 'vendors', 'expenses.customer_id = vendors.id', 'left' );
		$this->db->join( 'accounts', 'expenses.account_id = accounts.id', 'left' );
		$this->db->join( 'expensecat', 'expenses.category_id = expensecat.id', 'left' );
		$this->db->join( 'staff', 'expenses.staff_id = staff.id', 'left' );
	...
	function create( $params ) {
		...
		$i = 0;
		foreach ( $items as $item ) {
		    //////Add amount in glaccount table//////////
			$account_amountnew=0;$account_amount=0;
			$account_id=$item[ 'account' ];
			$this->db->select( 'amount' );
			$this->db->from( 'glaccounts' );
			$this->db->where('account_id',$account_id);
			$query=$this->db->get();
			if ($query->num_rows()>0) {
				$accountdata=$query->row();
				$account_amount=$accountdata->amount;
			}
			$account_amountnew=$account_amount+ $item[ 'quantity' ] * $item[ 'price' ] + ( ( $item[ 'tax' ] ) / 100 * $item[ 'quantity' ] * $item[ 'price' ] ) - ( ( $item[ 'discount' ] ) / 100 * $item[ 'quantity' ] * $item[ 'price' ] );
			$this->db->where('account_id', $account_id)->update( 'glaccounts', array('amount' => $account_amountnew) );
			////// End Add amount in glaccount table//////////
			$this->db->insert( 'items', array(
				'relation_type' => 'expense',
				'relation' => $expense,
				'product_id' => $item[ 'product_id' ],
	...
	function get_all_expenses_by_privileges($staff_id='') {
		$this->db->select( '*,vendors.company as customer,accounts.name as nameaccount,vendors.type as type,vendors.namesurname as individual,expensecat.name as category,staff.staffname as staff,expenses.description as desc, expenses.id as id' );
		$this->db->join( 'vendors', 'expenses.customer_id = vendors.id', 'left' );
		$this->db->join( 'expensecat', 'expenses.category_id = expensecat.id', 'left' );
		$this->db->join( 'staff', 'expenses.staff_id = staff.id', 'left' );
		$this->db->join( 'accounts', 'expenses.account_id = accounts.id', 'left' );
	...
	function get_expenses_by_privileges( $id, $staff_id='' ) {
		$this->db->select( '*,vendors.company as customer,vendors.vendor_number as vendor_number,accounts.name as account,expensecat.name as category,staff.staffname as staff,expenses.description as desc, expenses.id as id, expenses.staff_id as staff_id, staff.email as staffemail' );
		$this->db->join( 'vendors', 'expenses.customer_id = vendors.id', 'left' );
		$this->db->join( 'accounts', 'expenses.account_id = accounts.id', 'left' );
		$this->db->join( 'expensecat', 'expenses.category_id = expensecat.id', 'left' );
		$this->db->join( 'staff', 'expenses.staff_id = staff.id', 'left' );
		//$this->db->join( 'projects', 'projects.id = staff.id', 'left' );
		$this->db->order_by( 'expenses.id', 'desc' );
		if($staff_id) {
			$this->db->where('(expense_created_by='.$staff_id.' OR expenses.staff_id='.$staff_id.')');
			return $this->db->get_where( 'expenses', array( 'expenses.id' => $id) )->row_array();

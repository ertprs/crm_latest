...
class Purchases_Model extends CI_Model {
	...
	function purchases_add( $params ) {
		...
		$this->db->insert( 'history_logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '<a href="staff/staffmember/' . $this->session->usr_id . '"> ' . $this->session->staffname . '</a> ' . lang( 'added' ) . '  <a href="purchases/purchase/' . $purchase . '">' . get_number('purchases',$purchase,'purchase','purchase'). '</a>.' ),
			'staff_id' => $loggedinuserid,
			'type'=>'Purchase',
			'vendor_id'=>$purchase
		) );
	...
	function update_purchases( $id, $params ) {
		...
		$this->db->insert( 'history_logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '<a href="staff/staffmember/' . $this->session->usr_id . '"> ' . $this->session->staffname . '</a> ' . lang( 'updated' ) . '  <a href="purchases/purchase/' . $id . '">' . get_number('purchases',$id,'purchase','purchase') . '</a>.' ),
			'staff_id' => $loggedinuserid,
			'type'=>'Purchase',
			'vendor_id'=>$purchase
		) );
	...
	function delete_purchases( $id, $number ) {
		...
		$loggedinuserid = $this->session->usr_id;
		$appconfig = get_appconfig();
		$this->db->delete('history_logs', array( 'vendor_id' => $id,'type'=>'Purchase'));
		$this->db->insert( 'logs', array(
			'date' => date( 'Y-m-d H:i:s' ),

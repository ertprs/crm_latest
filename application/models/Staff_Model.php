...
class Staff_Model extends CI_Model {
	...
	function get_all_staff($type="",$staid="") {
		if($type) $this->db->where("staff.status",$type);
		if($staid) $this->db->where("staff.id",$staid);
		...
		function get_all_staff_own($type="",$staid="") {
		if($type) $this->db->where("staff.status",$type);
		if($staid){
		    $this->db->where("staff.id",$staid);
		    $this->db->or_where("staff.user_id",$staid);
	...
	function get_all_staff_active($type="",$staid="") {
		if($type) $this->db->where("staff.status",$type);
		if($staid) $this->db->where("staff.id",$staid);
		$this->db->order_by("staff.id ","desc");
		$this->db->select( '*,departments.name as department, status.name as statusname,status.color as statuscolor,staff.id as id' );
		$this->db->where("staff.staff_status",'1');
		$this->db->join( 'departments', 'staff.department_id = departments.id', 'left' );
		$this->db->join( 'status', 'staff.status = status.id', 'left' );
		return $this->db->get_where( 'staff', array( 'is_staff'=>'1' ) )->result_array();
	}
	...
	function get_all_staff_news($type="") {
		if($type) $this->db->where("staff.staff_status",$type);
		$this->db->order_by("staff.staffname ","asc");
		$this->db->select( '*,departments.name as department, staff.id as id' );
		$this->db->join( 'departments', 'staff.department_id = departments.id', 'left' );
		$this->db->join( 'status', 'staff.status = status.id', 'left' );

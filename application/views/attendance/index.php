              ...
              <div class="row">
                 <div class="col-md-12">
                    <div style="border-top-left-radius: 10px;" class="ciuis-right-border-b1 ciuis-invoice-summaries-b1">
                       <div class="box-header text-uppercase text-bold">Active</div>
                       <div class="box-content" style="width: 130px; height: 130px;">
                          <div class="percentage" ng-bind="openLeads" id="active">
                          </div>
                    ...
                    <div style="border-top-left-radius: 10px;" class="ciuis-right-border-b1 ciuis-invoice-summaries-b1">
                       ...
                       <div class="box-content" style="width: 130px; height: 130px;">
                          <div class="percentage cursor button1"  id="present" data-id="Present" >
                          </div>
                          <canvas id="0" width="130" height="130" style="border: 1px solid;border-radius: 50%;"></canvas>
                       </div>
                    </div>
                    <div class="ciuis-right-border-b1 ciuis-invoice-summaries-b1">
                       ...
                       <div class="box-content invoice-percent" style="width: 130px; height: 130px;">
                         <div class="percentage cursor button1" data-id="Absent" id="absent" ></div>
                          <canvas id="0" width="130" height="130" style="border: 1px solid;border-radius: 50%;"></canvas>
                       </div>
                    </div>
                  <!--  <div class="ciuis-right-border-b1 ciuis-invoice-summaries-b1">
                    ...
                    <div class="ciuis-right-border-b1 ciuis-invoice-summaries-b1">
                       <div class="box-header text-uppercase text-bold">EARLY OUT </div>
                       <div class="box-content invoice-percent-2" style="width: 130px; height: 130px;">
                          
                          <div class="percentage cursor button2" data-id="EarlyOut" id="earlyout" ></div>
                    ...
                    <div style="border-top-right-radius: 10px;" class="ciuis-invoice-summaries-b1">
                       ...
                       <div class="box-content invoice-percent-3" style="width: 130px; height: 130px;">
                          <div class="percentage cursor"  id="vacation"></div>
...
$next_day_continuation = $ci->Attendance_Model->next_day_continuation($details["punch_date"],$details["empl_id"]);
	$next_day_prstatus = $ci->Attendance_Model->next_day_prstatus($details["punch_date"],$details["empl_id"]);
...
foreach($break_timings as $break){
    ...
    if($time1 < $br_start && $et_time > $br_end){
...
//echo ($time1.'--'.strtotime($timings['start_time']));
    if($time1 < $st_time  && $time2 <= $et_time){
...
$timediff= $this->Attendance_Model->get_time_difference($details['CheckOutTime'],$timings['start_time']);
	...
	else if($time1 < $st_time  && $time2 >= $et_time){
	    
$ottimediff= $this->Attendance_Model->get_time_diff($details['CheckOutTime'],$out_end_grace);
      
       
$timediff= $this->Attendance_Model->get_time_difference($timings['end_time'],$timings['start_time']);
	$pr_status = 'Present';
	}
...
//$difference = round(abs($time2 - $time1) / 3600,2);
     ...
     if($details['CheckOutTime'] == '' && $next_day_continuation == '1'){
	  if($details['CheckInTime'] < $timings['end_time']){
	    ...
	    $ottimediff = $this->Attendance_Model->get_time_difference('24.00',$timings['end_time']);
 ...
 else if($details['CheckOutTime'] == '' && $next_day_prstatus == '1'){
								    if($details['punch_date'] != date('Y-m-d')){
								    $details['CheckOutTime'] = $timings['end_time'];
									 $timediff= $this->Attendance_Model->get_time_difference($timings['end_time'],$details['CheckInTime']);
								    }
									 else{
									$timediff = '-';
 ...
 if($details['CheckInTime'] == '00:00:00'  && $details['CheckOutTime'] == '' && $details['type_of_leave'] != ''){
							    
							    if($details['payment_type'] == 'Paid'){
							        $timediff= '-';
								$ottimediff = '-';
							        	$pr_status = '('.$details['type_of_leave'].')';
							        if($details['type_of_leave'] ==	'Annual Leave'){
							            $vacation_count ++;
							            
							        }
							        		$present++;
							    }else if($details['payment_type'] == 'Unpaid'){
							         $timediff= '-';
								$ottimediff = '-';
							        	$pr_status = 'Absent';
							    }
 ...
 if($timediff!='-'){
    ...
    else if($details['CheckInTime'] < $timings['start_time'] && $details['CheckInTime'] !='00:00:00' && $details['CheckInTime'] < $in_start_grace && $details['CheckInTime'] != '' ) {
        echo '<font color="Yellow"><b>Early In</b></font>' ;
    }
    else if($details['CheckInTime'] == '00:00:00' || $details['CheckInTime'] == ''){
        echo '';    
...
$ontime++;
        ...
        if($details['CheckOutTime'] >= $out_end_grace ){ echo '<b><font color="red">Late Out</font></b>' ; }
				       ...
				       $earlyout ++;
				       echo '<b><font color="red">Early Out</font></b>'; }  else if($details['CheckOutTime'] == '') { echo '<b><font color="black">-</font></b>';  } }else{
...
$('#employee_id').select2({ });
    ...
    $(document).ready(function(){
	$('#active').html('<?php print $act_count;?>');
		$('#present').html('<?php print $present;?>');
		$('#absent').html('<?php print $absent;?>');
		$('#earlyout').html('<?php print $earlyout;?>');
		$('#vacation').html('<?php print $vacation_count;?>');
      var date_input=$('.newdatepicker'); //our date input has the name "date"
      var container=$('.bootstrap-iso form').length>0 ? $('.bootstrap-iso form').parent() : "body";

...
<div class="ciuis-body-content" ng-controller="Expense_Controller">
  <div class="main-content container-fluid col-xs-12 col-md-12 col-lg-9">
    ...
    <md-toolbar ng-show="!expensesLoader" class="toolbar-white" ng-cloak>
      <div class="md-toolbar-tools">
        <h2 class="md-pl-10" flex md-truncate>
          ...
          <span class="badge"><strong ng-bind="expense.customer_name"></strong></span>
        </h2>
    ...
    <md-content ng-show="!expensesLoader" class="bg-white invoice" layout-padding ng-cloak>
		...
		<div class="ciuis-ticket-fieldgroup col-sm-3">
			<div class="ticket-label"><?php echo 'Vendor' ?></div>
			<div class="ticket-data" > <span ng-bind="expense.customer_name"></span></div>
		</div>
		<div class="ciuis-ticket-fieldgroup col-sm-3">
			<div class="ticket-label"><?php echo lang('expense'). ' '.lang('date')?></div>
			<div class="ticket-data" > <span ng-bind="expense.date | date:'dd, MMMM yyyy EEEE'"></span></div>
		</div>
		<div class="ciuis-ticket-fieldgroup col-sm-3">
			<div class="ticket-label"><?php echo 'Reference' ?></div>
			<div class="ticket-data" > <span ng-bind="expense.number"></span></div>
		</div>
		<div class="ciuis-ticket-fieldgroup col-sm-3" ng-show="expense.amountType==1">
			<div class="ticket-label" >Cash</div>
			<div class="ticket-data" > <span ng-bind="expense.account_name"></span></div>
		</div>
		<div class="ciuis-ticket-fieldgroup col-sm-3" ng-show="expense.amountType==2">
			<div class="ticket-label" >Card</div>
			<div class="ticket-data" > <span ng-bind="expense.card_id"></span></div>
		</div>
		<div class="ciuis-ticket-fieldgroup col-sm-3" >
			<div class="ticket-label" ><?php echo lang('created')?></div>
			<div class="ticket-data" > <span ng-bind="expense.created"></span></div>
		</div>
		<div class="ciuis-ticket-fieldgroup col-sm-3" >
			<div class="ticket-label" ><?php echo lang('staff')?></div>
			<div class="ticket-data" >
				<a ng-href="<?php echo base_url('staff/staffmember/')?>{{expense.staff_id}}">
					<span ng-bind="expense.staff_name"></span>
				</a>
			</div>
		</div>
      ...
      <div class="invoice-content col-md-12 md-p-0 xs-p-0 sm-p-0 lg-p-0">
		...
		<div class="invoice-price">
			<div class="invoice-price-right">
				<div class="col-sm-12 pull-right"><label  class="col-sm-10"><b><?php echo lang('subtotal') ?></b></label>  <label class="col-sm-2" ng-bind-html="expense.sub_total | currencyFormat:cur_code:null:true:cur_lct"></label></div>
				<div class="col-sm-12 pull-right"> <label  class="col-sm-10"><b><?php echo lang('tax') ?></b></label>  <label class="col-sm-2" ng-bind-html="expense.total_tax | currencyFormat:cur_code:null:true:cur_lct"></label></div>  
				<div class="col-sm-12 pull-right"> <label class="col-sm-10"><b><?php echo lang('total') ?></b></label>    <label class="col-sm-2" ng-bind-html="expense.total | currencyFormat:cur_code:null:true:cur_lct"></label></div>
			</div>
		</div>
    ...
    <md-content ng-show="!expensesFiles" class="bg-white">
      <md-list flex ng-cloak>
        <md-list-item class="md-2-line" ng-repeat="file in files | pagination : currentPage*itemsPerPage | limitTo: 6">
          ...
          <?php if (check_privilege('expenses', 'delete')) { ?>
            <md-button class="md-secondary md-primary" ng-click='DeleteFile(file.id)' aria-label="call">
...
<script> 
  ...
  lang.delete_expense = "<?php echo lang('expensesatentiondetail')?>";
  lang.convert_title = "<?php echo lang('convert').' '.lang('expense').' '.lang('to').' '.lang('invoice')?>";
  lang.convert_text = "<?php echo lang('convertmsg').' '.lang('expense').' '.lang('to').' '.lang('invoice')?>";
  lang.convert = "<?php echo lang('convert')?>";
  lang.email_sent_success = "<?php echo lang('email_sent_success') ?>";
</script>
<script type="text/ng-template" id="addfile-template.html">
  <md-dialog aria-label="options dialog">
  <?php echo form_open_multipart('expenses/add_file/'.$expenses['id'].'',array("class"=>"form-horizontal","enctype"=>"multipart/form-data")); ?>

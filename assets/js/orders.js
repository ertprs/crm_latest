function Orders_Controller($scope, $http, $mdSidenav, $q, $timeout, $filter) {
	...
	$scope.ChangeColor = function(type) {
		...
		$('.OrderStatus_'+type).css('color','orange');
	}
	...
	$http.get(BASE_URL + 'orders/get_orders').then(function (Orders) {
		...
		$scope.GetQuotesbyFilter = function(_prop, status) {
...
function Order_Controller($scope, $http, $mdSidenav, $mdDialog, $q, $timeout,$element,fileUpload) {
	...
	$scope.closeconvertedmarkas = function () {
		$mdDialog.hide();
	};
	...
	$http.get(BASE_URL + 'orders/get_order/' + ORDERID).then(function (OrderDetails) {
		...
		if(cnt > 0) {
			$scope.showdiscounttab = 1;
		}
		console.log($scope.showdiscounttab);
		console.log(cnt);
		...
		$scope.is_tentative = "0";
		$scope.deadlinedate = moment($scope.deadlinedate).format("YYYY-MM-DD");
	
		$scope.convertProject = function (ev) {
			$mdDialog.show({
				templateUrl: 'project-converted-template.html',
				scope: $scope,
				preserveScope: true,
				targetEvent: ev
			});
		...
		$scope.orderconvertedtoproject = function() {
			var dataObj = $.param({
				order_id: ORDERID,
				is_tentative: $scope.is_tentative,
				deadline_date : moment($scope.deadlinedate).format("YYYY-MM-DD")
			});
			var posturl = BASE_URL + 'orders/convert_project/'+ORDERID;
			$http.post(posturl, dataObj, config)
			.then(
				function (response) {
					console.log(response);
		...
		$scope.saveAll = function () {
			var dataObj = $.param({
				...
				reference: $scope.order.reference,

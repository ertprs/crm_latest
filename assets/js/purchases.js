function Purchases_Controller($scope,$filter, $http, $mdSidenav, $q, $timeout) {
	"use strict";
	...
	$scope.Getmaterial = function (searchText) {
		...
		/* $timeout(function () {
			deferred.resolve($scope.materials);
	...
	$scope.Getvendordata=function(vendor){
		$http.get(BASE_URL + 'purchases/get_VendorData/'+vendor).then(function (Vendors) {
			$scope.credit_period = Vendors.data;
			 $scope.duedate='';
            var sdate = $scope.created;
            var newdate = new Date(sdate);
            newdate.setDate(newdate.getDate()+parseInt($scope.credit_period));
...
function Purchase_Controller($scope, $http, $mdSidenav, $mdDialog, $q, $timeout) {
	...
	/* $scope.Getmaterial = (function (search) {
		var deferred = $q.defer();
		$timeout(function () {
			deferred.resolve($scope.materials);
		}, Math.random() * 500, false);
 ...
 */
	...
	$http.get(BASE_URL + 'api/doclogs/'+PURCHASEID+'/" "/Purchase').then(function (Logs) {
		$scope.logs = Logs.data;
	});
	$scope.loadMoreLogs = function() {
		$scope.getLogs = true;
		$http.get(BASE_URL + 'api/doclogs/'+PURCHASEID+'/loadMore/Purchase').then(function (Logs) {
			$scope.logs = Logs.data;
			$scope.getLogs = false;
		});
	}
